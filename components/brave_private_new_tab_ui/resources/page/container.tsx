...
import { useTorObserver, useHasSeenDisclaimer } from './hooks'
...
function Container () {
  ...
  const { hasSeenDisclaimer } = useHasSeenDisclaimer()
  ...
  const handleSearchSubmit = (value: string) => {
    getPageHandlerInstance().pageHandler.goToBraveSearch(value)
  }

  const handleDialogClose = () => {
    if (hasSeenDisclaimer) return
    getPageHandlerInstance().pageHandler.setHasUserSeenDisclaimerPref(true)
  }
    ...
    <BackgroundView isTor={isWindowTor}>
      ...
      <DialogBox>
        <DisclaimerDialog
          ...
          isOpen={false ?? !hasSeenDisclaimer}
