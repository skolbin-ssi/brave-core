...
TEST_F(BatAdsRotatingHashUserDataTest, GetRotatingHash) {
  ...
  AdvanceClock(TimeFromString("2 June 2022 11:00", /* is_local */ false));

  // Act
  const std::string json = GetRotatingHashAsJson();

  // Assert
  const std::string expected_json = R"({"rotating_hash":"2169836143"})";
...
TEST_F(BatAdsRotatingHashUserDataTest, RotatingHashMatchesBeforeNextHour) {
  ...
  AdvanceClock(TimeFromString("2 June 2022 11:000", /* is_local */ false));
  const std::string json_before = GetRotatingHashAsJson();

  // Act
  AdvanceClock(base::Hours(1) - base::Seconds(1));
